...
//    simultaneously acquired images with synchronized timestamps Depending on
//    the initial PTP state of each camera, it can take about 40 seconds for all
//    devices to autonegotiate.
...
AC_ERROR SynchronizeCamerasAndTriggerImage(acSystem hSystem, acDevice* phDevices, size_t numDevices)
{
	...
	for (size_t i = 0; i < numDevices; i++)
	{
		...
		err = acNodeMapSetBooleanValue(TLStreamNodeMap, "StreamPacketResendEnable", true);
		if (err != AC_ERR_SUCCESS)
			return err;
