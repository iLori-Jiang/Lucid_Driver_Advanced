...
THREAD_FUNCTION_SIGNATURE(EnumerationThread)
{
	...
	while (g_isRunning)
	{
		...
		while(g_isRunning  && !g_pDevice)
		{
			...
			// Search for device
			//    When the device has been disconnected, this thread waits for it to
			//    reconnect, constantly updating and scanning the device list for the
			//    lost device.
			...
			if (err == AC_ERR_SUCCESS)
			{
				...
				if (err == AC_ERR_SUCCESS && numDevices > 0)
				{
					...
					if (err == AC_ERR_SUCCESS)
					{
						...
						//    C_ForceIP), (3) running ArenaConfig to configure the
